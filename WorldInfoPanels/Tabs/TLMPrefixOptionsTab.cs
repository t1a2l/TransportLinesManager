...
namespace TransportLinesManager.WorldInfoPanels.Tabs
{
    internal class TLMPrefixOptionsTab : UICustomControl, IUVMPTWIPChild
    {
        ...
        public void Awake()
        {
            ...
            LogUtils.DoLog("Name");
            m_prefixName = CreateMiniTextField("TLM_PREFIX_NAME", OnPrefixNameChange);


            LogUtils.DoLog("ColorForModel");
            m_useColorForModel = m_helper.AddCheckboxLocale("TLM_USE_PREFIX_COLOR_FOR_VEHICLE", false, OnUseColorVehicleChange);
            MonoUtils.LimitWidthAndBox(m_useColorForModel.label, 340, true);
            ...
            m_paletteDD = CreateMiniDropdown("TLM_PALETTE", SetPalettePrefix, new string[1]);
            ReloadPalettes();
        ...
        private UIDropDown CreateMiniDropdown(string localeId, OnDropdownSelectionChanged onValueChanged, string[] values)
        {
            ...
            //MonoUtils.InitButtonFull(ddObj, false, "OptionsDropboxListbox");
            ddObj.horizontalAlignment = UIHorizontalAlignment.Center;
        ...
        private void ReloadPalettes()
        {
            ...
            m_paletteDD.items[0] = Locale.Get("TLM_USE_DEFAULT_PALETTE");
            m_isLoading = false;
        }
        private void CreateColorSelector()
        {
            ...
            MonoUtils.CreateUIElement(out UIButton resetColor, container.transform, "PrefixColorReset", new Vector4(290, 0, 0, 0));
            MonoUtils.InitButton(resetColor, false, "ButtonMenu");
            MonoUtils.LimitWidthAndBox(resetColor, 80, true);
        ...
        public void OnSetTarget(Type source)
        {
            ...
            m_title.text = string.Format(Locale.Get("TLM_PREFIX_TAB_WIP_TITLE"), sel > 0 ? NumberingUtils.GetStringFromNumber(TLMPrefixesUtils.GetStringOptionsForPrefix(TransportSystem), (int)sel + 1) : Locale.Get("TLM_UNPREFIXED"), TransportSystem.GetTransportName());
